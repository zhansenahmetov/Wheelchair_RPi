...
# This function initializes the SoC function (Coulomb counting function)
def initialize_SoC(vbat1, vbat2):
    # We can add a current condition here (i.e., -0.5 < ibat < 0.7)
    # This function initializes the SoC function (Coulomb counting function)
    ...
    # Execute the following if statement once to initialize the SoC function
    myVar[0] = vbat1             # myVar[0] is the open-circuit voltage of the 1st battery
    ...
    # Terminate function use (use only once)	
    initialize_SoC.__code__ = (lambda x,y:None).__code__

# This function should be called in the main code in the get_I2C() function
def SoC(sd_time, dt, ibat, vbat1, vbat2, rest_time, rest_time_limit):
    # This function should be called in the main code in the get_I2C() function
    # sd_time ~ the last time the Pi was shutdown appropriatly 
    # dt ~ sampling periode of the battery current
    # vbat1 ~ voltage of the battery pack
    # vbat2 ~ voltage of the lower/second battery 
    # rest_time ~ cumulative time variable in [seconds] that increments if the battery current is zeros 
    # rest_time_limit ~ the battery rest time in [minutes] (usually 1 hour or 45 minutes is sufficient)
    ...
    reset_time = 0  #Reset battery rest time
    ...
    if rest_time/60 >= rest_time_limit:
        myVar[0] = vbat1             # myVar[0] is the open-circuit voltage of the 1st battery
        myVar[1] = vbat2             # myVar[1] is the open-circuit voltage of the 2nd battery
        myVar[2] = ((vbat1-vbat2) - p2)/p1   # myVar[2] is the SoC1 (Capacity) of the 1st battery
        myVar[3] = (vbat2 - p2)/p1   # myVar[3] is the SoC2 (Capacity) of the 2nd battery
    ...
    with open('SoC.pkl', 'wb') as file:
        pickle.dump(myVar, file)
    return myVar[2], myVar[3], reset_time
...
def get_I2C():
    ...
    tx = gdt # at the beggining set it to 0.2 seconds.
