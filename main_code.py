...
months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov',
          'Dec']  # Month codes as stored in the RPi OS
...
def get_I2C():
    ...
    tx = time.time() - 0.135;  # at the beginning set it to 0.2 seconds, 0.135 since 0.065 seconds taken to get all data
    ...
    while True:
        ...
        if i == 25:  # every 5 sec
            ...
            file1.write(
                str(format(round(Avgvbat1, 3), '.3f')) + "," + str(format(round(Avgvbat2, 3), '.3f')) + "," + str(
                    format(round(Avgibat, 3), '.3f')) + "," + str(format(round(AvgTbat_1, 3), '.3f')) + "," + str(
                    format(round(AvgTbat_2, 3), '.3f')) + "," + str(format(round(AvgAmbTemp, 3), '.3f')) + "," + str(
                    format(round(AvgSoc, 3), '.3f')))
            file1.close()
...
def get_serial():
    ...
    writer2.writerow(["CPU Time", "Time", "GPS time", "Latitude", "Lat. Direction", "Longitude", "Long. Direction",
                      "Num. Satellites", "Speed(km/h)"])
